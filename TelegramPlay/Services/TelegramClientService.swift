...
final class TelegramClientService: ObservableObject {
    ...
    private static func playableMedia(from message: Message) -> PlayableMedia? {
        ...
        case .messageAnimation(let messageAnimation):
            let animation = messageAnimation.animation
            let file = animation.animation
            let name = animation.fileName.isEmpty ? "animation.mp4" : animation.fileName
            return PlayableMedia(
                id: message.id,
                fileId: file.id,
                fileName: name,
                remoteSize: file.size,
                title: messageAnimation.caption.text.isEmpty ? "GIF / animation" : messageAnimation.caption.text,
                durationSeconds: animation.duration
            )
        case .messageDocument(let messageDocument):
            let doc = messageDocument.document
